...
function setChartData(data) {
  ...
  const blockedCount = data.filter(customer => customer.blocked === 'BLOCKED').length
  const notBlockedCount = data.length - blockedCount

  return {
    labels: ['Unblocked Customers', 'Blocked Customers'],
    datasets: [
      {
        label: 'Blocked Customers',
        data: [notBlockedCount, blockedCount],
